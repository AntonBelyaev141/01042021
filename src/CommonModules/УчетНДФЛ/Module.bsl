...
Функция АБ_РассчитатьНалогПоОсновнойСтавке(ИсключаемыйРегистратор, Организация, МесяцРасчета, Знач СписокФизическихЛиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, ОкончательныйРасчет = Истина, ДатаУдержанияИсчисленногоНалога = Неопределено, ДвиженияТекущегоРегистратора = Неопределено) Экспорт
	#Вставка
	...
	Если ЕстьДвиженияТекущегоРегистратораПоДоходам Тогда
		...
		Для Каждого ТекущаяСтрока Из ДоходыТекущегоРегистратора Цикл
			...
			Если СоответствиеФизическихЛиц[ТекущаяСтрока.ФизическоеЛицо] = Истина
				...
				Если ОкончательныйРасчет И ЗначениеЗаполнено(ТекущаяСтрока.ЗарегистрированоПриНачисленииАванса) Тогда
					// Учтем при расчете фактическую дату получения дохода по выплаченному авансу.
					...
					НоваяСтрока.ИспользоватьДляРаспределения = Ложь;
