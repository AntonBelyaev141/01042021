...
//			ЗачтеноАвансовыхПлатежейСПревышения
...
//
Функция АБ_РассчитатьНалогПоОсновнойСтавке(ИсключаемыйРегистратор, Организация, МесяцРасчета, Знач СписокФизическихЛиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, Знач ОкончательныйРасчет = Истина, Знач ДатаУдержанияИсчисленногоНалога = Неопределено, ДвиженияТекущегоРегистратора = Неопределено) Экспорт
	#Вставка
	...
	// 1.1	Если расчет не затрагивает 2025 года, воспользуемся предыдущей реализацией алгоритма расчета
	ПервыйМесяцТекущегоАлгоритма = ДатаЗакона176ФЗ();
	Если ПоследнийМесяцНалоговогоПериода < ПервыйМесяцТекущегоАлгоритма Тогда
		#Вставка
		//1АБ Беляев 14.03.2021 +
		//Да, это действительно релиз 3_1_31_32
		...
		//1АБ Беляев 14.03.2021 -
		#КонецВставки
		Возврат РассчитатьНалогДо2025Года(ИсключаемыйРегистратор, Организация, МесяцРасчета, СписокФизическихЛиц, ИсходныйПервыйМесяцНалоговогоПериода, ИсходныйПоследнийМесяцНалоговогоПериода, ОкончательныйРасчет, ДатаУдержанияИсчисленногоНалога, ДвиженияТекущегоРегистратора)
	КонецЕсли;
	...
	Если ПервыйМесяцНалоговогоПериода < ПервыйМесяцТекущегоАлгоритма Тогда
		#Вставка
		//1АБ Беляев 14.03.2021 +
		//Да, и это тоже (релиз 3_1_31_32)
		ДвиженияТекущегоРегистратора = Новый Структура("АБ_НеПредоставлятьВычеты,ДвиженияТекущегоРегистратора",АБ_НеПредоставлятьВычеты,ДвиженияТекущегоРегистратора);
		//1АБ Беляев 14.03.2021 -
		#КонецВставки
		РезультатыПрошлыхЛет = РассчитатьНалогДо2025Года(ИсключаемыйРегистратор, Организация, МесяцРасчета, СписокФизическихЛиц, ИсходныйПервыйМесяцНалоговогоПериода, ПервыйМесяцТекущегоАлгоритма - 1, ОкончательныйРасчет, ДатаУдержанияИсчисленногоНалога, ДвиженияТекущегоРегистратора)
	КонецЕсли;
	ПервыйМесяцНалоговогоПериода = Макс(ПервыйМесяцНалоговогоПериода, ПервыйМесяцТекущегоАлгоритма);
    ...
    СтрокаНалогов = "НалогПоСтавке13," + РесурсыИсчисленногоНалогаСтрокой("Налог", Истина);
	МассивНалогов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаНалогов);
	...
	Если ОкончательныйРасчет Тогда
		...
		Если ЗначениеЗаполнено(ФизическиеЛицаДляЧтенияДополнительныхАвансов) Тогда
			...
			|	ФизическоеЛицо,
			|	Период";
			Запрос.Текст = СтрЗаменить(ТекстЗапроса, "&УсловиеОблагаемостиУНалоговогоАгента", УсловиеОблагаемостиУНалоговогоАгента());
	...
	Иначе
		...
		#Вставка
		|	И (&Условие_АБ_НеПредоставлятьВычетыВМежрасчет)
		#КонецВставки
		|
		|СГРУППИРОВАТЬ ПО
		|	СтрокиИсчисленногоНалога.ФизическоеЛицо,
		...
		Если АБ_НеПредоставлятьВычеты Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие_АБ_НеПредоставлятьВычетыВМежрасчет", "Ложь");
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие_АБ_НеПредоставлятьВычетыВМежрасчет", "Истина");
		КонецЕсли;
		#КонецВставки
		ИсчисленныйРанееНалог = Запрос.Выполнить().Выгрузить();
	...
	Если ЕстьНовыеЗаписи Тогда // Сторно-записи регистрируются достаточно редко, поэтому проверим, надо ли сворачивать таблицу.
		ИсчисленныйРанееНалог.Свернуть("Подразделение, ФизическоеЛицо, МесяцНалоговогоПериода, МесяцНалоговогоПериодаДляПоиска, КатегорияДохода", СтрокаНалогов);	
	КонецЕсли;	
	...
	Если ЕстьДвиженияТекущегоРегистратораПоДоходам Тогда
		...
		НеДобавлятьДанныеАвансов = Не ДоходыУчитываютсяТолькоПоДатеВыплаты(МесяцРасчета, -1);
		Для Каждого ТекущаяСтрока Из ДоходыТекущегоРегистратора Цикл
			...
			Если СоответствиеФизическихЛиц[ТекущаяСтрока.ФизическоеЛицо] <> Неопределено
				...
				И (ТекущаяСтрока.МесяцНалоговогоПериода >= ПервыйМесяцТекущегоАлгоритма Или ЗначениеЗаполнено(ТекущаяСтрока.ЗарегистрированоПриНачисленииАванса)) Тогда
	...
	ВспомогательнаяТаблицаМесяца = РезультатРасчета.СкопироватьКолонки("ФизическоеЛицо, МесяцПериодаПредоставленияВычета, МесяцНалоговогоПериода, Подразделение, КатегорияДохода, " 
		+ СтрокаНалогов + ", ЗачтеноАвансовыхПлатежейПоСтавке13, " + РесурсыИсчисленногоНалогаСтрокой("ЗачтеноАвансовыхПлатежей", Истина));
    ...
    СтрокаКатегорииНалогов = "КатегорияДохода, " + СтрокаНалогов;
		...
		Пока ИндексТекущейСтрокиТаблицыДоходов <= ДанныеУчетаДоходов.Количество() - 1 Цикл
			...
			Пока ПродолжатьРасчет Цикл
				...
				Если ТекущаяСтрокаТаблицыДоходов.ЭтоПерваяСтрокаЗаМесяц Тогда
					...
					ВписатьВРезультатЗарегистрированныеВУчетеДанные(ВспомогательнаяТаблицаМесяца, СтрокиНалога, СтрокаКатегорииНалогов);	
				...
				Если ТекущаяСтрокаТаблицыДоходов.ЭтоПоследняяСтрокаЗаМесяц Тогда				
					...
					Если Не ОкончательныйРасчетДляТекущегоФизическогоЛица И Не ЗначениеЗаполнено(ДатыТекущегоРасчетаПоМесяцам[Месяц(МесяцНалоговогоПериода)]) И Не ЗначениеЗаполнено(ДатыТекущегоРасчетаНерезидента[Месяц(МесяцНалоговогоПериода)]) Тогда
						...
						Если МесяцНалоговогоПериода < КонецМесяца(МесяцРасчета) Тогда
							ПроверяемаяСумма = 0;
							Для каждого ИмяКолонки Из МассивНалогов Цикл
								ПроверяемаяСумма = ПроверяемаяСумма	+ ВспомогательнаяТаблицаМесяца.Итог(ИмяКолонки)
							КонецЦикла;
					...
					Иначе 
						...
						Для каждого ИмяКолонки Из МассивНалогов Цикл
							ИсчисленоНДФЛЗаГод = ИсчисленоНДФЛЗаГод + ВспомогательнаяТаблицаМесяца.Итог(ИмяКолонки)
						КонецЦикла;
						...
						Если ПроводитьРасчетЗаОчереднойМесяц Тогда
							...
							ТаблицаМесяцаБезКатегорийДохода.Свернуть("МесяцНалоговогоПериода, Подразделение", СтрокаНалогов);
							Для каждого СтрокаТЗ Из ТаблицаМесяцаБезКатегорийДохода Цикл
								
								ОбнулятьСтроки = Ложь;
								Для каждого ИмяКолонки Из МассивНалогов Цикл
									ОбнулятьСтроки = СтрокаТЗ[ИмяКолонки] = 0;
									Если ОбнулятьСтроки Тогда
										Прервать;
									КонецЕсли;
								КонецЦикла;
								...
								Если ОбнулятьСтроки Тогда
									...
									ПроверяемаяСумма = 0;
									Для каждого ИмяКолонки Из МассивНалогов Цикл
										ПроверяемаяСумма = ПроверяемаяСумма	+ СтрокаТЗ[ИмяКолонки] * СтрокаТЗ[ИмяКолонки]
									КонецЦикла;
									Если ПроверяемаяСумма = 0 Тогда
							...
							Если ОкончательныйРасчетДляТекущегоФизическогоЛица Тогда
								Если Не ЗначениеЗаполнено(ОсновнаяДатаПолученияДохода) Тогда // перерасчет налога при отсутствии дохода текущего акта расчета
									...
									Если МесяцРасчета < ДатаЗакона263ФЗ() И МесяцНалоговогоПериода > КонецМесяца(МесяцРасчета) Тогда // перерасчет по будущим доходам не производим
										ДобавлятьРезультатМесяцаВИтогиФизлица = Ложь;
									ИначеЕсли МесяцРасчета >= ДатаЗакона263ФЗ() И МесяцНалоговогоПериода > ?(ЗначениеЗаполнено(ДатаУдержанияИсчисленногоНалога), КонецМесяца(ДатаУдержанияИсчисленногоНалога), ПоследнийМесяцНалоговогоПериода) Тогда // перерасчет по будущим доходам не производим
										ДобавлятьРезультатМесяцаВИтогиФизлица = Ложь;
			...
			Если ТекущаяСтрока.ФизическоеЛицо <> ТекущееФизическоеЛицо Тогда 
				...
				Если ТекущееФизическоеЛицо <> Неопределено Тогда  // Завершение обработки предыдущего физлица.
					...
					Для СчМесяцев = 1 По Месяц(МесяцНалоговогоПериодаПредыдущейСтроки) Цикл
						...
						ВписатьВРезультатЗарегистрированныеВУчетеДанные(ВспомогательнаяТаблицаМесяца, ИсчисленныйРанееНалог.НайтиСтроки(СтруктураПоискаИсчисленного), СтрокаКатегорииНалогов);	
					КонецЦикла;
