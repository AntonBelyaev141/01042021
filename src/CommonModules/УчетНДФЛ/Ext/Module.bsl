...
//
Функция АБ_РассчитатьНалогПоОсновнойСтавке(ИсключаемыйРегистратор, Организация, МесяцРасчета, Знач СписокФизическихЛиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, Знач ОкончательныйРасчет = Истина, ДатаУдержанияИсчисленногоНалога = Неопределено, ДвиженияТекущегоРегистратора = Неопределено) Экспорт	
	#Вставка
	...
	ПервыйМесяцТекущегоАлгоритма = ДатаЗакона263ФЗ();
	Если ПоследнийМесяцНалоговогоПериода < ПервыйМесяцТекущегоАлгоритма Тогда
		#Вставка
		//1АБ Беляев 14.03.2021 +
		//Да, это действительно релиз 3_1_27_51
	...
	Если ПервыйМесяцНалоговогоПериода < ПервыйМесяцТекущегоАлгоритма Тогда
		#Вставка
		//1АБ Беляев 14.03.2021 +
		//Да, и это тоже (релиз 3_1_27_51)
		ДвиженияТекущегоРегистратора = Новый Структура("АБ_НеПредоставлятьВычеты,ДвиженияТекущегоРегистратора",АБ_НеПредоставлятьВычеты,ДвиженияТекущегоРегистратора);
		//1АБ Беляев 14.03.2021 -
		#КонецВставки
		РезультатыПрошлыхЛет = РассчитатьНалогДо2023Года(ИсключаемыйРегистратор, Организация, МесяцРасчета, СписокФизическихЛиц, , ПервыйМесяцТекущегоАлгоритма - 1, ОкончательныйРасчет, ДатаУдержанияИсчисленногоНалога, ДвиженияТекущегоРегистратора)
	КонецЕсли;
	...
	Если ОкончательныйРасчет Тогда
		...
		ФизическиеЛицаБезАвансаВДвиженияхТекущегоРегистратора = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(СписокФизическихЛиц);
		...
		Если ЭтоРасчетПоДоходамТекущегоМесяца Тогда
			Если ЕстьДвиженияТекущегоРегистратораПоДоходам Тогда
				...
				Для каждого Элемент Из СписокФизическихЛиц Цикл
					...
					КонецЕсли;
				КонецЦикла; 
				ФизическиеЛицаДляЧтенияДополнительныхАвансов = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ФизическиеЛицаСАвансомВДвиженияхТекущегоРегистратора);
			Иначе
				ФизическиеЛицаБезДвиженийТекущегоРегистратора = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(СписокФизическихЛиц);
			КонецЕсли;
			Если ЗначениеЗаполнено(ФизическиеЛицаБезДвиженийТекущегоРегистратора) Тогда
				...
				|ГДЕ
				|	Авансы.Организация = &Организация
		...
		Если ЗначениеЗаполнено(ФизическиеЛицаДляЧтенияДополнительныхАвансов) Тогда
			...
			|			И СведенияОДоходахНДФЛ.Регистратор = ДокументыУчтенныеПриРасчетеНДФЛ.ДокументОснование
			|ГДЕ
			|	СведенияОДоходахНДФЛ.Период МЕЖДУ &НачалоМесяцаРасчета И &КонецМесяцаРасчета
			|	И СведенияОДоходахНДФЛ.ГоловнаяОрганизация = &ГоловнаяОрганизация
			|	И СведенияОДоходахНДФЛ.Регистратор <> &ИсключаемыйРегистратор
			|	И СведенияОДоходахНДФЛ.МесяцНалоговогоПериода МЕЖДУ &НачалоГодаПериодаРасчета И &КонецПериодаРасчета
			|	И (СведенияОДоходахНДФЛ.КодДохода.СтавкаНалогообложенияРезидента = ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13)
	...
	Иначе
		...
		// (в форме временной таблиы для использования в запросах сбора данных и в форме соответствия для алгоритма расчета)
		...
		|	-РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СуммаСПревышения КАК НалогСПревышенияПоСтавке13,
		...
		|		ПО СтрокиДоходов.Регистратор = РегистраторыОкончательногоРасчета.Регистратор
		|ГДЕ
		...
		|	СУММА(СтрокиИсчисленногоНалога.НалогСПревышенияПоСтавке13) КАК НалогСПревышенияПоСтавке13
		|ИЗ
		|	ВТСтрокиИсчисленногоНалога КАК СтрокиИсчисленногоНалога
		...
		|	СведенияОДоходахНДФЛ.ДоходТекущегоРасчета КАК ДоходТекущегоРасчета
		|ИЗ
		...
		|	СведенияОДоходахНДФЛ.ДоходТекущегоРасчета
		|
		|ИМЕЮЩИЕ
		|	(СУММА(СведенияОДоходахНДФЛ.СуммаДохода) <> 0
		|		ИЛИ СУММА(СведенияОДоходахНДФЛ.СуммаВычета) <> 0)";
	...
	Иначе
		...
		|						И ИмущественныеВычеты.Период <= &КонецМесяцаРасчета
		|			ИНАЧЕ ИмущественныеВычеты.Период <= &КонецПериодаРасчета
		|					И (ИмущественныеВычеты.Период < &НачалоМесяцаРасчета
