#Область Доработка_6_2_ОтменитьПереопределениеДатПолученияДоходаИзВедомостей
&ИзменениеИКонтроль("ЗарегистрироватьНовуюДатуПолученияДохода")
...
//
Процедура АБ_ЗарегистрироватьНовуюДатуПолученияДохода(Регистратор, Движения, МенеджерВременныхТаблиц, ДатаВыплаты, ДатаОперации, Отказ, Записывать = Ложь) Экспорт
	...
	#Вставка
	//1АБ Беляев 19.03.2021 +
	КоличествоСтрок=Движения.РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Количество();
	Для НомСтроки=-1*КоличествоСтрок по -1 Цикл
		ИндексТекСтроки=-1*НомСтроки-1;
		ТекСтрока=Движения.РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Получить(ИндексТекСтроки);
		Если ТекСтрока.ВидДвижения=ВидДвиженияНакопления.Приход Тогда
			Движения.РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Удалить(ТекСтрока);
		КонецЕсли;
	КонецЦикла;
	//1АБ Беляев 19.03.2021 -
	#КонецВставки
