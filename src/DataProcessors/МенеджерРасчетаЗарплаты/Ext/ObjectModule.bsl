...
Функция АБ_ПоказателиСотрудников() Экспорт
	...
	ПроцентДоплатыЗаНочные=ПоказательПроцентДоплатыЗаНочные;
	ПроцентДоплатыЗаВечерние=ПоказательПроцентДоплатыЗаВечерние;
	//1АБ Беляев 08.03.2021 -
	#КонецВставки	
	...
	Для Каждого ТекущаяСтрокаНачислений Из Начисления Цикл
		...
		Для Каждого ТекущийПоказатель Из ПоказателиТекущегоНачисления Цикл
			...
			ИначеЕсли (ТекущийПоказатель.Показатель = ПоказательСреднийЗаработокОбщий
				Или ТекущийПоказатель.Показатель = ПоказательСреднийЗаработокИндексируемый
				Или ТекущийПоказатель.Показатель = ПоказательСреднийЗаработокНеиндексируемый)
